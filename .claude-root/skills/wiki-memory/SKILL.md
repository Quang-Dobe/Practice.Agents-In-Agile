...
- **Never** write into `.claude-root/` (this toolset, including this skill, the agents,
...
`.claude-root/templates/memory-topic.md` and carries exactly **two REQUIRED fields** and
nothing else:
...
- If `docs/memory/<slug>.md` **does not exist** → **create** it from
  `.claude-root/templates/memory-topic.md` (a `# <Title>` heading, a one-line summary, a
...
Serialization (matches `.claude-root/templates/memory-topic.md`):
...
1. **Build the candidate entry.** Assemble the one-line summary and the `source-ref:`
   ...
   `.claude-root/templates/memory-topic.md`). Do not add a timestamp, a
...
6. **Else create-or-append.**
   - If `docs/memory/<slug>.md` does not exist → create it from
     `.claude-root/templates/memory-topic.md`, then insert the entry at the end of its
     `## Entries`.
...
A `.claude-root/skills/wiki-memory/SKILL.md` is **well-formed for write** iff **both** of
the following hold:
