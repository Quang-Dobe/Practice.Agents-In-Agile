...
output format. Discover the optional `architecture-rules` / `coding-rules` seams via `project-seams`. When a
question turns on what a library can actually do, follow `library-knowledge` before you write it —
an answerable question is not an open question.
