---
description: Bootstrap docs/domain/ DDD wiki from a fresh repository
argument-hint: <path> [branch-name]
---

Bootstrap the DDD domain wiki under `docs/domain/` of the working directory by exploring a fresh repository at `<path>`.
...
4. Spawn the `project-explorer` subagent via the `Agent` tool with `description: "project-explorer: bootstrap docs/domain/"` and a `prompt` containing: the resolved absolute `<path>`, the `[branch-name]` arg if supplied (or the literal `null` if omitted), and the instruction that the agent must reload `.claude/skills/project-explorer/SKILL.md` before any other action.

5. The subagent will execute its `## Operating procedure` (idempotency guard, skill load, repo walk, BC candidate surfacing, APPROVE gate, output generation, frontmatter recording) and produce the Evans-canonical tree under `docs/domain/`.
