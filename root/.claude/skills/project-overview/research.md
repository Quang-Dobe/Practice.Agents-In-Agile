...
No new external sources are introduced for v1. The narrative file shape is agent-internal, specified in `SKILL.md` `## Output schema`. For the MediatR / vertical-slice .NET ecosystem citations (Bogard, `IRequest` / `IRequestHandler` / `INotification` shapes, anemic-domain-model commentary) that this skill's BC detection and stub-fallback heuristics reuse by reference, see the full `## Sources` block in the `project-explorer` skill's `research.md`. The narrative skill does not duplicate those citations — it consumes them on every reload via the sibling skill.
...
BC detection rules are not re-derived here. See the `project-explorer` skill's `research.md` `## DDD code signals` (5 categories: aggregates, repositories, events, services, value objects) and `## Ubiquitous-language extraction heuristic` (named heuristic: weighted cross-boundary symbol frequency). The `project-overview` agent reuses these heuristics verbatim — narrative-tree authoring shifts the *output shape*, not the *detection rules*.
...
This skill **mirrors** — no longer inherits — the `project-explorer` skill `## BC candidate surfacing` (and its H3 subsections `### Grouping rule`, `### Candidate report format`, `### Small-repo fallback detection`) plus `### Auto-write contract`, and cites the `project-explorer` skill's `research.md` `## DDD code signals` + `## Ubiquitous-language extraction heuristic` by reference. The `project-overview` agent stopped preloading the sibling skill: carrying a 27k-character skill in the preamble of every request of every bootstrap run was the largest avoidable cost in the pipeline, so `SKILL.md` `## BC candidate surfacing` now states those contracts in full. The consequence is that drift is no longer impossible by construction. The editor of either skill is obligated to mirror the change into the other in the same commit, and to re-read this file end-to-end plus the sibling `SKILL.md` `## BC candidate surfacing` section before considering the edit complete. The `project-update` agent still preloads both skills, so it is where drift surfaces first. No `skill_version` pin or `compatible_with` check exists or is planned — this paragraph is the trip-wire.
