...
1. **Mode gate (replaces D11).** Detect the grounding mode in the working dir:
   ...
   When **both** project and root signals exist, **project mode wins** (per-BC wiki is richer than the root rollup). Carry the detected mode + its grounding paths into every `present-builder` spawn so the diagram unit (`overview-plan`) reads the right source.
