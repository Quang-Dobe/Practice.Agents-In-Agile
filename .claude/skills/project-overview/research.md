...
BC detection rules are not re-derived here. See `.claude/skills/project-explorer/research.md` `## DDD code signals` (5 categories: aggregates, repositories, events, services, value objects) and `## Ubiquitous-language extraction heuristic` (named heuristic: weighted cross-boundary symbol frequency). The `project-overview` agent reuses these heuristics verbatim — narrative-tree authoring shifts the *output shape*, not the *detection rules*.
