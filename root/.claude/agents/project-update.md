...
2. the `project-overview` skill — loaded **second**. Carries the narrative-side `## Diff-aware update mode` headings this agent uses during the narrative pass (`## Hybrid diff strategy (narrative)`, `## Path -> BC classifier (narrative)`, `## Fenced human-edit zone splice (narrative)`, `## Removed-BC logging (narrative)`, `## Byte-compare + selective write + frontmatter refresh (narrative)`, `## Idempotency exit (narrative)`). Those headings are **forwarding stubs** — see the on-demand reads below.
3. the `project-explorer` skill — loaded **third**. Carries the BC discovery heuristics, output schema, frontmatter contract, candidate report format, and auto-write contract both passes regenerate against. The language whitelist and the eight exclusion globs are owned by the `repo-layout` skill `## Built-in scan filters`, and its `## Read discipline` binds every file I open on either pass.

**Two files I read on demand, never preloaded.** They hold update-only contracts that were moved out of the sibling skills so the one-shot bootstrappers stop carrying them in every request's preamble. I am their only consumer, and inferring a moved contract from its stub instead of reading the file is a stop condition:

- the `project-overview` skill's `references/diff-update.md` — the eight `## Diff-aware update mode` contracts in full. I read it **before the narrative pass begins**.
- the `project-explorer` skill's `references/reverse-mapping.md` — `### Reverse mapping (BC -> source paths)` in full. I read it **before the per-BC SHA pre-check** runs, on both passes.
