...
2. **Local-paths-only guard.** If `[root-path]` matches `^https?://` or `^git@`, refuse
   ...
   (NFR-6, mirroring `.claude/commands/project/explore.md`.)
...
4. **Input discovery (read-only).** All discovery is structure-based, not name-prefix-based
   ...
   - **Ignore** the root's own `docs/` directory and the `.claude-root/` directory — they
   ...
   - **Architecture file:** locate the root `docs/architecture.md`. It is an independent,
     ...
     Get-ChildItem -Path $rootPath -Directory `
       | Where-Object { $_.Name -notin @('docs', '.claude-root') } `
       | Where-Object {
...
5. **Forward-dependency stop-condition (write-discipline manual).** The write rules
   ...
   **not** inlined here. They live in `.claude-root/skills/wiki-memory/SKILL.md` (authored
...
6. **Spawn the worker (primary shape).** Spawn the `wiki-bootstrapper` subagent via the
   ...
   must reload `.claude-root/skills/wiki-memory/SKILL.md` before any other action. This
   ...
   **Inline fallback.** If the worker agent is unavailable, the command may perform the
   work itself: reload `.claude-root/skills/wiki-memory/SKILL.md` first (honoring step 5's
...
7. **Build topics — summarize-and-link (no verbatim copies).** Shape every topic against
   `.claude-root/templates/memory-topic.md`: one `# <Topic title>` heading, exactly one
...
9. **Write — additive, confined to `docs/memory/`.** On exact-case `APPROVE`, the write is
   **create-or-additive only** (locked decision 2):
   ...
   - Every write lands **only** under `docs/memory/`. The command **never** writes to
     ...
     or `.claude-root/` (FR-6, NFR-2). Link only to source files that exist; never invent a
