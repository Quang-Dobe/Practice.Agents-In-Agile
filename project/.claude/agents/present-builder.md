...
description: Project-tier runtime agent. Builds the per-unit present-*.html files for a feature from its planning artifacts (and the wiki, for diagram units). Gate-free; runs in project mode (docs/domain+docs/narrative) or root mode (docs/architecture.md+docs/memory) — /present:build detects the mode and passes it in.
