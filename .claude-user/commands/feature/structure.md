...
- **Live-spawn note.** The four named subagents (`business-analyst`, `architect`, `software-engineer`) must be present in `.claude-user/agents/`. They are loaded by Claude Code at session start; if you renamed or replaced any of them mid-session, restart the session before running this command.
