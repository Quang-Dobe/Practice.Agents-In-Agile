...
Usage:
    ...
    uv run python render_excalidraw.py <path-to-file.excalidraw> --svg [--output path.svg]

--svg writes the exported SVG markup instead of a raster screenshot, with the diagram
background turned off so a host page's own ground shows through.
...
def render(
    ...
    """Render an .excalidraw file to PNG, or to SVG markup when as_svg. Returns the output path."""
    ...
    if output_path is None:
        output_path = excalidraw_path.with_suffix(".svg" if as_svg else ".png")
    ...
    with sync_playwright() as p:
        ...
        opts_str = json.dumps({"transparent": as_svg})
        ...
        if as_svg:
            markup = page.eval_on_selector("#root svg", "el => el.outerHTML")
            if not markup:
                print("ERROR: Exported SVG markup was empty.", file=sys.stderr)
                browser.close()
                sys.exit(1)
...
def main() -> None:
    ...
    parser.add_argument("--output", "-o", type=Path, default=None, help="Output path (default: same name with .png, or .svg with --svg)")
    parser.add_argument("--scale", "-s", type=int, default=2, help="Device scale factor, PNG only (default: 2)")
    parser.add_argument("--width", "-w", type=int, default=1920, help="Max viewport width (default: 1920)")
    parser.add_argument("--svg", action="store_true", help="Write exported SVG markup with no background instead of a PNG screenshot")
    ...
    out_path = render(args.input, args.output, args.scale, args.width, args.svg)
    print(str(out_path))
