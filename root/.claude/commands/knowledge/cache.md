...
4. For each targeted library, in order:
   ...
   - `cache` path exists as a file on disk AND its frontmatter `detected_version` equals the
     `detected_version` recorded for this library in `tech-stack.md` (not a re-parse of the
     dependency manifest — this command never touches the dependency manifest at all) → skip,
     print `<name>: cache already fresh (detected_version <version>).`
