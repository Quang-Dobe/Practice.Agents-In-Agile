...
per-repo `.claude/` agents (`project-explorer`, `project-overview`,
...
I reload `.claude-root/skills/wiki-memory/SKILL.md` at the **start of every run** and treat
...
- **Discovered qualifying-repo set** — the depth-1 child directories of the root that each
  contain `docs/narrative/` OR `docs/domain/`. The root's own `docs/` and `.claude-root/`
  are excluded.
...
1. **Reload the skill.** Load `.claude-root/skills/wiki-memory/SKILL.md`; honor the
...
2. **Discover inputs (read-only).** Confirm the depth-1 qualifying repos (narrative OR
   ...
   `.claude-root/`. Do not recurse below depth 1.
...
4. **Summarize-and-link (no verbatim copies).** Shape each topic against
   `.claude-root/templates/memory-topic.md`. The substantive content is a one-line summary
...
- **No write outside `docs/memory/`.** Never write to `docs/architecture.md`, any repo's
  `docs/narrative/`/`docs/domain/`, repo source, or `.claude-root/` (FR-6, NFR-2).
