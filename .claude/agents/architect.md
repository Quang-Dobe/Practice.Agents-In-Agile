...
3. **Inject a `## N. Test Strategy` section before the Approval Checklist.** The section MUST contain a 4-column markdown table:

   ```
   | Step ID | Goal | Test kind | Owner |
   |---|---|---|---|
   | A | <goal of Step A copied from overview-plan> | <concrete test instruction OR literal `skip Tester`> | <`Tester` or `—`> |
   | B | ... | ... | ... |
   ```

   Exactly one row per implementation step (`Step A`, `Step B`, …) in `overview-plan.md`. Prose is not an acceptable substitute. Per R7 (see `.claude/skills/architect/architect.md`).
