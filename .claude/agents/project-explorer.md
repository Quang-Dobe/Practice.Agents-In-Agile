...
- (c) **Skill file missing or malformed.** `.claude/skills/project-explorer/SKILL.md` cannot be read, its YAML frontmatter does not parse, or required body sections (`## Operating procedure`, `## Code signals`, `## Output schema`, `## Frontmatter contract`) are absent. I stop before step 3 of the operating procedure.
...
- Do not re-run, diff, or merge against an existing `docs/domain/` tree. Those semantics belong to the deferred sibling `project-wiki-enhancer`.
- Do not attempt to verify the `[branch-name]` arg against the target repo's actual git state (it is recording-only per D6).
